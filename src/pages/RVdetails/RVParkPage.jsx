...
// // const RVParkPage = () => {
// //   const { data: reservation, isError, isLoading } = useGetRvCampsitesQuery();
...
// //   return (
// //     <div className="py-16 px-4 bg-gray-100 space-y-8">
// //       {reservationData?.map((rvCampsitesDetailsData) => {
// //         return (
// //           <div key={rvCampsitesDetailsData?._id} className="max-w-7xl mx-auto">
...
// //                       {rvCampsitesDetailsData?.amenities?.map(
// //                         (amenity, index) => (
...
// //                 attractions and top-notch amenities, your RV getaway starts
// //                 here.
// //               </p>
...
// // export default RVParkPage;
// import React, { useState } from "react";
// import { Navigate, useNavigate } from "react-router-dom";
// import { Spin, Modal, Row, Col, Card } from "antd";
// import { useGetRvCampsitesQuery } from "../../services/api/pages/rvCampsitesSlice";
...
//   const [currentCampsiteId, setCurrentCampsiteId] = useState(null);

//   const handleBookSpot = (spot, campsiteId) => {
//     setSelectedSpot(spot);
//     setCurrentCampsiteId(campsiteId);
...
//     setCurrentCampsiteId(null);
//   };

//   const handleContinueBooking = () => {
//     setIsModalVisible(false);
//     navigate(`/checkout/${currentCampsiteId}`, {
...
//             </p>
//             <a
//               href={`https://www.google.com/maps?q=${selectedSpot.lat},${selectedSpot.long}`}
//               target="_blank"
//               rel="noopener noreferrer"
...
import React, { useState } from "react";
import { Navigate, useNavigate } from "react-router-dom";
import { Spin, Modal, Row, Col, Card, DatePicker, Tag } from "antd";
...
const RVParkPage = () => {
  ...
  const isSpotAvailable = (spot, checkIn, checkOut) => {
    ...
    const isBooked = spot.availability.booked.some(
      (booking) =>
        checkOutDate.isAfter(dayjs(booking.from), "day") &&
        checkInDate.isBefore(dayjs(booking.to), "day")
    );
    ...
    const isAvailable = spot.availability.available.some((avail) => {
      ...
      return (
        checkInDate.isSameOrAfter(fromDate, "day") &&
        checkOutDate.isSameOrBefore(toDate, "day")
      );
    });
      ...
      >
        {selectedSpot ? (
          <div className="space-y-4">
            ...
            <p>
              <strong>Availability Status:</strong>{" "}
              {selectedSpot.isAvailable ? "Available" : "Unavailable"}
            </p>
            <p>
              <strong>Status:</strong>{" "}
              {selectedSpot.status === 1 ? "Active" : "Inactive"}
            </p>
            <div>
              <strong>Available Dates:</strong>
              <div className="mt-1 flex flex-wrap gap-2">
